...
function Slackbot(configuration) {
    ...
    slack_botkit.createWebhookEndpoints = function(webserver, authenticationTokens) {
        ...
        slack_botkit.log(
            ...
            'webhooks at: https://' + slack_botkit.config.hostname + ':' + slack_botkit.config.port + '/slack/receive');
    ...
    slack_botkit.setupWebserver = function(port, cb) {
        ...
        var server = slack_botkit.webserver.listen(
            ...
            function() {
				slack_botkit.log('** Static Dir ' + static_dir + ' **');
                slack_botkit.log('** Starting webserver on ' + slack_botkit.config.hostname + ':' + slack_botkit.config.port);
    ...
    slack_botkit.createOauthEndpoints = function(webserver, callback) {
        ...
        var call_api = function(command, options, cb) {
            ...
            if (proxyUrl) {
                var params = {timeout: 3000, proxy: proxyUrl};
			} else {
				var params = null;
			}
            
			request.post('https://slack.com/api/' + command, params, function(error, response, body) {
                slack_botkit.debug('2-Got  response', error, body);
